...
authkey 参数密钥，验证args
**/
function flashupload(uploadid, name, textareaid, funcName, args, action, catid, authkey) {
    var args = args ? '&args=' + args : '';
    var setting = '&a=' + action + '&catid=' + catid ;
    Wind.use("artDialog","iframeTools",function(){
        art.dialog.open(GV.DIMAUB+'index.php?m=asset&g=asset' + args + setting, {
        title: name,
        id: uploadid,
        width: '650px',
        height: '420px',
...
function change_images(uploadid, returnid) {
    ...
    $.each(contents, function(i, n) {
        ...
        str += "<li id='image" + ids + "'><img src='" + n + "' style='width:100px;' /><input title='双击查看' type='hidden' name='" + returnid + "_url[]' value='" + n + "' style='width:310px;' ondblclick='image_priview(this.value);' class='input'> <input type='text' name='" + returnid + "_alt[]' value='" + filename + "' style='width:160px;' class='input' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\"> <a href=\"javascript:remove_img('image" + ids + "')\">移除</a> </li>";
    });
...
function remove_img(id){
    //先通过后端删除图片的原始地址，再移除前端div层
    var imgurl=$('#' + id + " input[name='photos_url[]']").val();
    $.ajax({
   type: "POST",
   url: GV.DIMAUB+'index.php?m=asset&g=asset&a=delete',
   data: "id=" + imgurl,
   cache: false,
