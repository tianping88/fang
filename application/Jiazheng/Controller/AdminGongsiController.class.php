...
class AdminGongsiController extends AdminbaseController{
	...
	public function edit(){
		...
		$post=$this->gongsi_model->where("id=$id")->find();
	...
	public function edit_gongsi(){
		...
		if (IS_POST) {
            ...
            if($data){
                $data['createdate']=strtotime($data['createdate']);                
                $data['update_time']=NOW_TIME;
                $result=$this->gongsi_model->save($data);
    			if (false!==$result) {
    				$this->success("保存成功！");
    			} else {
    				$this->error("保存失败！");
	...
	private  function _lists($status="all"){
		...
		$where_ands=array();
		if(!empty($term_id)) $where_ands=array("item = $term_id");
		if("all"!==$status){
		  array_push($where_ands, "status = '$status'");
		}else{
		  array_push($where_ands, "status > -1 ");
		}
	...
	function delete(){
		if(isset($_GET['id'])){
			$tid = intval(I("get.id"));
			$data['status']=-1;
			if ($this->gongsi_model->where("id=$tid")->save($data)!==false) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
		...
		if(isset($_POST['ids'])){
			$tids=join(",",$_POST['ids']);
			$data['status']=-1;
			if ($this->gongsi_model->where("id in ($tids)")->save($data)!==false) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
	...
	function check(){
		if(isset($_POST['ids']) && $_GET["check"]){
			...
			if ( $this->gongsi_model->where("id in ($ids)")->save($data)!==false) {
				$this->success("审核成功！");
			} else {
				$this->error("审核失败！");
			}
	...
	function clean(){
		if(isset($_POST['ids'])){
			...
			$status=$this->gongsi_model->where("id in ($tids)")->delete();			
			...
			if(isset($_GET['id'])){
				...
				$status=$this->gongsi_model->where("id = $id")->delete();				
				if ($status!==false) {
					$this->success("删除成功！");
				} else {
					$this->error("删除失败！");
	...
	function restore(){
		if(isset($_GET['id'])){
			...
			if ($this->gongsi_model->save($data)) {
				$this->success("还原成功！");
			} else {
				$this->error("还原失败！");
			}
