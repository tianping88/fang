...
class PublicController extends AdminbaseController {
    ...
    public function dologin(){
    	...
    	if(!sp_check_verify_code()){
    		...
    		if(!empty($result)){//这里如果只允许管理员登录后台的话，将if(!empty($result) && $result['user_type']==1){
    			if($result['user_pass'] == sp_password($pass)){
    				...
    				if( $result["id"]!=1 && ( empty($groups) || empty($result['user_status']) ) ){//只允许管理员登录后台，将if( $result["id"]!=1 && ( empty($groups) || empty($result['user_status']) ) ){
